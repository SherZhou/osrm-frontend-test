...
for (const filepath of [leafletOptions, debug]) {
  ...
  const CENTER = process.env.OSRM_CENTER || '39.9,116.39'
  ...
  if (CENTER) {
    ...
    // Mapbox uses LngLat
    if (options.match('116.39,39.9')) options = options.replace('116.39,39.9', lnglat)
    // Leaflet uses LatLng
    else options = options.replace('39.9,116.39', latlng)
  }
