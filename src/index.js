...
var LRM = require('./leaflet_routing_machine');
...
//路由部分添加到地图
var lrmControl = LRM.control(Object.assign(controlOptions, {
  router: router
})).addTo(map);
...
btn.addEventListener('click', function () {
  ...
  // 在控制台输出参数值，你可以根据需要进行其他操作
  console.log('参数1:', param1);
  console.log('参数2:', param2);
  console.log('参数3:', param3);
  // console.log('参数4:', param4);

  // console.log(tmp);
  // lrmControl.
  //修改osrm配置参数 ，但仍然是与定义好的url交互
  // L.routing.osrmv1.bind(...)
  //要搞清楚url拼接和请求cors(lrm的osrm,搞懂复现)的调用逻辑，找到可以二次修改的入口
  
  var url = "http://localhost:5000/route/v1/mytest/" + param1;
  console.log('url:', url);
  var xhr = corslite(url, L.bind(function (err, resp) {
    ...
    timer = setTimeout(function () {
      ...
      callback.call(context || callback, {
        status: -1,
        message: 'OSRM test request timed out.'
      });
  ...
  try {
    // console.log('Waypoint:'+lrmControl.getWaypoints());
    var tmp = lrmControl.getWaypoints();
    for(var i in tmp) {
      //第一个是起点
      console.log(i,"in test:",tmp[i],",",tmp[i].latLng);
     }
     var lastpnt = tmp[1].latLng;
     console.log("lastpnt:",lastpnt);
    map.fire('click', { latlng: lastpnt});
  } catch (error) {
    console.log('fire failed:'+error.message);
  }
  console.log('click fire finished');
